...
def Y1():
    ...
    def Y_34_price(klfg):       
        ...
        def Group_kap():
            ...
            return A
        def for_y5():
            N2 = int(st.number_input('Введите последний год из числа лет, предшествующих году, в который произошел пожар', key=str(klfg)+'5000005'), step = 1)
            C3 = st.number_input('Введите кадастровую стоимость объекта строительства', key=str(klfg)+'55')
            N1 = int(st.number_input('Введите год, в котором была произведенна оценка стоимости объекта строительства', key=str(klfg)+'5089775', step = 1))
...
def y13():
    ...
    def y13_11(w):
        ...
        s1 = st.number_input('введите уничтоженную пожаром площадь i-го объекта строительства, кв.м. определяемую в соответствие с порядком заполнения и представления КУП', key=str(w)+'83453457')
