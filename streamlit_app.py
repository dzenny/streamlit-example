...
def Y1():
    ...
    def Y_34_price(klfg):
        ...
        S3 =st.number_input('Введите повреждённую пожаром площадь i-го объекта строительства кв. м.', key=str(klfg)+'5345343846335')
    ...
    #начали цену 
        if vibor_A == 0:
            ...
            if st.checkbox('дополнительный расчёт', key=str(klfg)+'5923738605'):
                N1 = st.number_input('год в который установленна кадастровая стоимость объекта', key=str(klfg)+'59256565355',min_value = 2000,max_value = 2021, step=1,)
                N2 = st.number_input('Год в котором произошёл пожар', key=str(klfg)+'52645654655',min_value = 2000,max_value = 2021, step=1,)
                pril_j = {
                    2000:1.202,
                    2001:1.1858,
                    2002:1.1506,
