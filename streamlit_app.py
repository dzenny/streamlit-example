...
def Y1():
    ...
    def Y_34_price(klfg):       
        ...
        def c1_pril():
            ...
            if vibor_ == 0:
                price = st.number_input('введите восстановительную стоимость 1 кв.м i-го объекта строительства, руб./кв.м.',min_value = 0.01, key=str(klfg)+'55')
            else:
                price = c1_A()
            return price
        def Fact_kap():
        ...
        def Group_kap():
            ...
            return A
        def for_y5():
            N2 = int(st.number_input('Введите последний год из числа лет, предшествующих году, в который произошел пожар', step = 1, key=str(klfg)+'5000005'))
            C3 = st.number_input('Введите кадастровую стоимость объекта строительства', key=str(klfg)+'55')
            N1 = int(st.number_input('Введите год, в котором была произведенна оценка стоимости объекта строительства',step = 1, key=str(klfg)+'5089775'))
        ...
        if vibral == 2:
            if GK == 10:
                S2 = st.number_input('Общая площадь объекта строительства',min_value = 0.01, key=str(klfg)+'544553377775')
                C2 = for_y5()
                y5 = C2/S2     
            else:
                FK = Fact_kap()
            ...
            if GK == 10:
                S2 = st.number_input('Общая площадь объекта строительства',min_value = 0.01, key=str(klfg)+'547678885')
                C2 = for_y5()
                y5 = (C2/S2)*K6     
            else:
                FK = Fact_kap()
                ...
                y5 = C1*k2*k3*k4*K6
            S3 =st.number_input('Введите повреждённую пожаром площадь i-го объекта строительства кв. м.',min_value = 0.01, key=str(klfg)+'534534333335')
            y3 = y5*S3
        return y3
...
def y13():
    ...
    def y13_11(w):
        ...
        k_un = {"Здание производственного назначения":12686,
        "Складское здание, сооружение (в т.ч. сельскохозяйственное здание, сооружение для хранения)":24586,
        "Одноквартирный жилой дом":14386,
        "Многоквартирный жилой дом":12946,
        "Другое здание, сооружение, строение жилого сектора (кроме забора)":6078,
        "Здание, сооружение сельскохозяйственного назначения (кроме зданий, сооружений для хранения)":32200,
        "Здание, сооружение общественного назначения":29439,
        "Другое здание, сооружение, строение (кроме сооружений, установок промышленного назначения, неэксплуатируемых, строящихся зданий, сооружений)":17010,
        "иное":0,
        }

        k_pov = {"Здание производственного назначения":9698,
        "Складское здание, сооружение (в т.ч. сельскохозяйственное здание, сооружение для хранения)":9840,
        "Одноквартирный жилой дом":3935,
        "Многоквартирный жилой дом":9271,
        "Другое здание, сооружение, строение жилого сектора (кроме забора)":5301,
        "Здание, сооружение сельскохозяйственного назначения (кроме зданий, сооружений для хранения)":8023,
        "Здание, сооружение общественного назначения":27739,
        "Другое здание, сооружение, строение (кроме сооружений, установок промышленного назначения, неэксплуатируемых, строящихся зданий, сооружений)":12763,
        "иное":0,
        }
        ...
        s1 = st.number_input('введите уничтоженную пожаром площадь i-го объекта строительства, кв.м. определяемую в соответствие с порядком заполнения и представления КУП',min_value =0.01, key=str(w)+'83453457')
        ...
        s3 = st.number_input('введите поврежденную пожаром площадь i-го объекта строительства, кв.м. определяемую в соответствии с Порядком заполнения и представления КУП',min_value =0.01, key=str(w)+'836367')   
        ...
        if y16 == 0:
            c13 = st.number_input('введите общую стоимость имущества, находившегося на момент пожара на i-м объекте строительства, руб.',min_value =0.01, key=str(w)+'8457')
            s2 = st.number_input('введите общую площадь i-го объекта строительства, кв.м. определяемую в соответствии с Порядком заполнения и представления КУП',min_value =0.01, key=str(w)+'87')       
            y14 = (c13/s2)*s1
            
        else:
            y14 = y16*s1
        if y25 == 0:
            c131 = st.number_input('введите общую стоимость имущества, находившегося на момент пожара на i-м объекте строительства, руб.',min_value =0.01, key=str(w)+'877')
            s2232 = st.number_input('введите общую площадь i-го объекта строительства, кв.м. определяемую в соответствии с Порядком заполнения и представления КУП',min_value =0.01, key=str(w)+'84477')
...
def y26():
    ...
    def y231(kraft):
        ...
        s14 = st.number_input('введите общую площадь i-го транспортного средства в кв. м.',min_value =0.01, key=str(kraft)+'443544544')
        s4 = st.number_input('введите уничтоженную в результате горения площадь i-го т/с. в кв. м.',min_value =0.01, key=str(kraft)+'42454554')
        s5 = st.number_input('введите повреждённую в результате горения площадь i-го т/с. в кв. м.',min_value =0.01, key=str(kraft)+'46786794')
